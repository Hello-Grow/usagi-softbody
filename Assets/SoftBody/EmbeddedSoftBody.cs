...
public class EmbeddedSoftBody : MonoBehaviour
{
    ...
    [Header("Compression Resistance")]
    [Tooltip("Keeps connected particles from being pushed closer than this fraction of their rest distance.")]
    [Range(0.1f, 1f)] public float minimumEdgeLengthRatio = 0.7f;
    [Min(0f)] public float compressionStrength = 3500f;
    [Min(0f)] public float compressionDamping = 100f;
    public float compressionMaxForce = Mathf.Infinity;

    [Header("Stretch Limit")]
    [Tooltip("Connected particles cannot separate beyond this multiple of their bind-pose distance.")]
    [Range(1f, 4f)] public float maximumEdgeLengthRatio = 1.5f;
    ...
    [Header("High-Speed Collision")]
    [Tooltip("Uses swept speculative contacts on every soft-body particle so fast moving or rotating colliders cannot tunnel through the body.")]
    public bool useContinuousParticleCollision = true;
    ...
    Vector3[] _restVertsWorld;
    Quaternion _visualFacingLocal = Quaternion.identity;
    ...
    CompressionEdge[] _compressionEdges;
    ...
    // Existing scenes contain already-created joints. Keep their drives in
    // sync with the component settings instead of only applying values when
    // the Context Menu creates a brand-new joint.
    void ApplyJointDrives()
    {
        ...
        {
            joints[i].xDrive = linearDrive;
            joints[i].yDrive = linearDrive;
            joints[i].zDrive = linearDrive;
            joints[i].slerpDrive = angularDrive;
        }
    }
    ...
    void LateUpdate()
    {
        ...
        if (_restVertsWorld == null || _restVertsWorld.Length != _restVertsLocal.Length) return;
    ...
    void FixedUpdate()
    {
        ApplyCompressionResistance();
        EnforceStretchLimit();
    }

    void ConfigureParticleCollision()
    {
        if (!useContinuousParticleCollision || _bodies == null) return;
        ...
        for (int i = 0; i < _bodies.Length; i++)
        {
            ...
            // Speculative CCD includes angular sweeps, unlike regular
            // ContinuousDynamic CCD. It is therefore the reliable mode for
            // particle spheres meeting spinning blades or another player.
            body.collisionDetectionMode = CollisionDetectionMode.ContinuousSpeculative;
            body.maxDepenetrationVelocity = Mathf.Max(body.maxDepenetrationVelocity, 100f);
        }
    }
    ...
    // ConfigurableJoint drives are elastic: a sufficiently large collision
    // impulse can still collapse a whole region. This one-sided constraint
    // adds a strong repulsion only after an edge has compressed past its
    // allowed fraction of the bind-pose length.
    void CacheCompressionEdges()
    {
        if (pointTransforms.Count < 2)
        {
            _compressionEdges = Array.Empty<CompressionEdge>();
            return;
        }

        BuildEdgesByKNN();
        var cachedEdges = new List<CompressionEdge>(edges.Count);
        foreach (Edge edge in edges)
        {
            Vector3 a = _restParticlePosW[edge.a];
            Vector3 b = _restParticlePosW[edge.b];
            float restLength = Vector3.Distance(a, b);
            if (restLength > 0.0001f)
                cachedEdges.Add(new CompressionEdge { a = edge.a, b = edge.b, restLength = restLength });
        }
        _compressionEdges = cachedEdges.ToArray();
    }

    void ApplyCompressionResistance()
    {
        if (_bodies == null || _compressionEdges == null || compressionStrength <= 0f)
            return;

        float ratio = Mathf.Clamp(minimumEdgeLengthRatio, 0.1f, 1f);
        for (int i = 0; i < _compressionEdges.Length; i++)
        {
            CompressionEdge edge = _compressionEdges[i];
            Rigidbody a = _bodies[edge.a];
            Rigidbody b = _bodies[edge.b];
            if (a == null || b == null) continue;

            Vector3 delta = b.worldCenterOfMass - a.worldCenterOfMass;
            float distance = delta.magnitude;
            float minimumLength = edge.restLength * ratio;
            if (distance >= minimumLength || distance < 0.0001f) continue;

            Vector3 direction = delta / distance;
            float closingSpeed = Mathf.Max(0f, Vector3.Dot(b.linearVelocity - a.linearVelocity, direction));
            float forceMagnitude = compressionStrength * (minimumLength - distance)
                + compressionDamping * closingSpeed;
            forceMagnitude = Mathf.Min(forceMagnitude, compressionMaxForce);

            Vector3 force = direction * forceMagnitude;
            if (!a.isKinematic) a.AddForce(-force, ForceMode.Force);
            if (!b.isKinematic) b.AddForce(force, ForceMode.Force);
        }
    }

    // Joint drives are elastic, so a strong enough impulse can pull two
    // particles farther apart than their intended connection length. Clamp
    // each cached edge after the physics solve to make the configured maximum
    // a real limit rather than only another spring force.
    void EnforceStretchLimit()
    {
        if (_bodies == null || _compressionEdges == null) return;

        float ratio = Mathf.Max(1f, maximumEdgeLengthRatio);
        for (int i = 0; i < _compressionEdges.Length; i++)
        {
            CompressionEdge edge = _compressionEdges[i];
            Rigidbody a = _bodies[edge.a];
            ...
            Vector3 delta = b.worldCenterOfMass - a.worldCenterOfMass;
            float distance = delta.magnitude;
            float maximumLength = edge.restLength * ratio;
            if (distance <= maximumLength || distance < 0.0001f) continue;

            Vector3 direction = delta / distance;
            float excess = distance - maximumLength;
            bool moveA = !a.isKinematic;
            bool moveB = !b.isKinematic;
            int movableBodies = (moveA ? 1 : 0) + (moveB ? 1 : 0);
            if (movableBodies == 0) continue;

            // Share the correction equally when both particles are dynamic;
            // otherwise keep the kinematic particle fixed.
            Vector3 correction = direction * (excess / movableBodies);
            if (moveA) a.position += correction;
            if (moveB) b.position -= correction;

            // Remove only velocity that would immediately stretch this edge
            // again; tangential motion stays untouched.
            float separatingSpeed = Vector3.Dot(b.linearVelocity - a.linearVelocity, direction);
            if (separatingSpeed > 0f)
            {
                Vector3 velocityCorrection = direction * (separatingSpeed / movableBodies);
                if (moveA) a.linearVelocity += velocityCorrection;
                if (moveB) b.linearVelocity -= velocityCorrection;
            }
        }
    }
    ...
    /// <summary>
    /// Applies an Inspector-style root rotation while keeping every dynamic
    /// particle at its matching rotated world position in the same step.
    /// </summary>
    public Quaternion RotateWholeBody(Quaternion targetRotation, float maxDegreesDelta)
    {
        ...
        Quaternion nextRotation = Quaternion.RotateTowards(
            ...
            targetRotation,
            maxDegreesDelta);
        Quaternion turn = nextRotation * Quaternion.Inverse(previousRotation);
        if (Quaternion.Angle(Quaternion.identity, turn) < 0.001f) return nextRotation;

        var oldCenters = new Vector3[pointTransforms.Count];
        var oldRotations = new Quaternion[pointTransforms.Count];
        Vector3 pivot = Vector3.zero;
        int bodyCount = 0;
        for (int i = 0; i < pointTransforms.Count; i++)
        {
            Rigidbody body = pointTransforms[i] != null
                ? pointTransforms[i].GetComponent<Rigidbody>()
                : null;
            oldCenters[i] = body != null ? body.worldCenterOfMass : Vector3.zero;
            oldRotations[i] = body != null ? body.rotation : Quaternion.identity;
            if (body != null)
            {
                pivot += oldCenters[i];
                bodyCount++;
        ...
        if (bodyCount > 0) pivot /= bodyCount;

        transform.rotation = nextRotation;
        // Rotate the root around the point cloud's centre, not its arbitrary
        // import origin. This keeps the player in place while it turns.
        transform.position = pivot + turn * (previousPosition - pivot);
        ...
        for (int i = 0; i < pointTransforms.Count; i++)
        {
            ...
            Vector3 targetCenter = pivot + turn * (oldCenters[i] - pivot);
            body.position += targetCenter - body.worldCenterOfMass;
            body.rotation = turn * oldRotations[i];
            body.linearVelocity = turn * body.linearVelocity;
        }
    ...
    /// <summary>
    /// Sets the visual facing without rotating the dynamic Rigidbody hierarchy.
    /// The mesh is rotated during skinning about the live point-cloud centre.
    /// </summary>
    public void SetVisualFacing(Quaternion worldFacing)
    {
        _visualFacingLocal = Quaternion.Inverse(transform.rotation) * worldFacing;
    }

    /// <summary>Converts a desired rendered world direction into the matching
    /// physics-space direction before the visual skin rotation is applied.</summary>
    public Vector3 RenderedDirectionToPhysicsDirection(Vector3 renderedWorldDirection)
    {
        Vector3 localDirection = transform.InverseTransformDirection(renderedWorldDirection);
        Vector3 physicsLocalDirection = Quaternion.Inverse(_visualFacingLocal) * localDirection;
        return transform.TransformDirection(physicsLocalDirection).normalized;
    }
    ...
    void DeformMeshSkinned()
    {
        ...
        Vector3 visualPivot = Vector3.zero;
        int pointCount = 0;
        for (int pi = 0; pi < pointTransforms.Count; pi++)
        {
            if (pointTransforms[pi] == null) continue;
            visualPivot += transform.InverseTransformPoint(pointTransforms[pi].position);
            pointCount++;
        }
        if (pointCount > 0) visualPivot /= pointCount;

        for (int vi = 0; vi < vCount; vi++)
        {
            // Always deform from the world-space bind pose. Re-transforming
            // the rest vertex by the current root transform here applies root
            // rotation twice: once here and again in the particle delta.
            Vector3 vRestW = _restVertsWorld[vi];
            ...
            Vector3 localVertex = transform.InverseTransformPoint(vOutW);
            deformed[vi] = visualPivot + _visualFacingLocal * (localVertex - visualPivot);
        }
